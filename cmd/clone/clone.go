...
func run(c *cobra.Command, _ []string) {
	...
	for _, repo := range dir.Repos {
		...
		if !nofork {
			...
			if err != nil {
				pullFromUpstreamActivity.EndWithFailure(err)
				logger.Printf("We weren't able to pull the latest upstream changes into your fork of %s. This is probably because you have a pre-existing fork with commits ahead of upstream. Please change this or delete your fork, and try again.", repo.FullRepoName)
				errorCount++
				continue
			}
